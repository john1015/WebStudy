...
// DBCP => 보안이 뛰어나다 / Connection을 효율적으로 관리 (POOL) / 연결에 소모되는 시간을 절약할 수 있다
// 한번에 접속자가 많이 있어도 쉽게 서버다운이 안된다
// DBCP => 웹 개발에에 일반화 (실무는 거의 대부분 => 98%) => 홈페이지 (일반 JDBC) , 공기업/금융권/포탈 (DBCP)
...
public class DataBoardDAO {
	...
	// static => 메모리 공간을 한개만 설정이 가능하다
	// 미리 연결해서 저장된 POOL안에 Connection주소를 얻어온다
	...
	public void disConnection() {
		...
		try {
			if(ps!=null) ps.close();
			if(conn!=null) conn.close(); // Connection 반환이 된다 => commons-dbcp.jar(Apache)
			// 																				 commons-pool.jar
			// => tomcat => lib => 저장
		} catch (Exception ex) {} 
	}
	...
	// 데이터가 삭제가 되기전에 파일 읽기 => 파일을 폴더에서 삭제
	...
	public String databoardDelete(int no , String pwd) {
		...
		try {
			getConnection();
			String sql="select pwd from databoard where no="+no;
			ps=conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			rs.next();
