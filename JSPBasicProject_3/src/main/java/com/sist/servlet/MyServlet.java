 ...
 *       															   (경량) -> Spring
 *       		=> Server + Applet
 *       						   ==== Application + let => 서버에서 실행되는 응용 프로그램
 *       							| Applet => 자바가 발전=> Applet
 *       							  ==== 웹 => 정보 공유 목적 (1995 이전 => 문자로만 출력)
 *       							  | 이미지 / 애니메이션 => 2000년 웹 => 전자상 거래
 *       		=> 웹 서비스 기능을 해주는 자바 클래스 
 *       			=> 자바안에 HTML코드를 첨부 (JSP => HTML안에 Java를 첨부)
 *       				  out.writr("<html>") => <% 
 *       													 %> 
 *       													 <html>
 *       			=> 단점 
 *                       1. HTML 사용하기가 어렵다(복잡) => CSS/JavaScript 화면 UI는 하지 않는다
 *                       	  out.wirte("<html>")
 *                       	  out.wirte("<head>")
 *                       	  out.wirte("<script type = \"text/javascript\">")
 *                       	  out.wirte("function = aaa(){")
 *                       	  out.wirte("alert(\"\");")
 *                       	  out.wirte("}")
 *                       	  out.wirte("</script>")
 *                       	  out.wirte("<style type=\"text/css\">")
 *                       	  out.wirte("#aaa{background-color:red; border:1px silid red;}")
 *                       	  out.wirte("</style>")
 *                       	  out.wirte("</head>")
 *                       	  out.wirte("</html>")
 *                       2. HTML / CSS / JavaScript에 대하여 에러처리가 어렵다
 *                       3. 확장자 java => 변경시마다 컴파일을 다시 해야된다
 *                       	 => 톰캣 연결
 *                       	 => 톰캣 프로젝트 => 변경 : 컴파일 => 톰캣에 올려준다
 *                       	 => AWS => 수정시마다 컴파일 => war => war로 변경해야 된다
 *                       	 	  ==================================
 *                       		  운영 (SE)  => SI / SM 
 *                       		  ============= 통합 DevOps => CI / CD
 *                       	 => 버늩 색상 변경
 *                       	 => 개발자 => 수정시에 바로 확인 => JSP
 *       			=> 장점
 *       					자바 => 배포시에 .class파일만 전송 => 보안이 뛰어나다
 *       							   ============== 전체 소스를 볼 수 없다 , 다른 사람이 변경이 불가능
 *       					주로 사용처 : HTML / 자바가 분리가 된 경우 => 연결해주는 역할
 *       									=========================== Controller
 *       																								   | 스프링은 이미 제작 (서블릿)
 *       			=> 단점을 보완
 ...
 *       					   ===
 *       						| 오라클이 인수 => package => jakarata => tmcat 10이상
 *       						STS 3.9 => jdk 11 => 실무 1.8(호환성이 좋다)
 *       						STS 4.X => jdk 17
...
public class MyServlet extends HttpServlet {
	 ...
	 *     							=> 데이터 출력
	 *     
	 *      		// 사용자가 데이터전송 GET ==> doGet() 호출
	 *      		// 사용자가 데이터전송 POST ==> doPost() 호출
	 *      		// 통합 : service()
	 *      
	 *       		// => JSON / XML => Ajax
	 *       										  javascript and xml
