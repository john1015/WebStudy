...
public class MemberDAO {
	 ...
	 * SELECT COUNT(*) FROM member WHERE id=#{id} </select> <select
	 * id="memberInfoData" resultType="MemberVO" parameterType="string"> SELECT *
	 ...
	 * project_member VALUES( #{id},#{pwd},#{name},#{sex},#{birthday},#{post},
	 * #{addr1},#{addr2},#{phone},#{email},#{content},SYSDATE,'n' ) </insert>
	 */
	 ...
	 * SELECT id,name,sex,birthday,post,addr1,addr2,phone,email,content FROM
	 * project_member WHERE id=#{id} </select>
	 */
	 ...
	 * SELECT pwd FROM project_member WHERE id=#{id} </select> <update
	 * id="memberUpdate" parameterType="MemberVO"> UPDATE project_member SET
	 ...
	 * SELECT COUNT(*) FROM project_member WHERE name=#{name} AND email=#{email}
	 * </select> <select id="memberIdFindData" resultType="String"
	 * parameterType="MemberVO"> SELECT RPAD(SUBSTR(id,1,2),LENGTH(id),'*') FROM
	 * project_member WHERE name=#{name} AND email=#{email} </select>
	 */
