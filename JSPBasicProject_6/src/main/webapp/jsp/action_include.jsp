	...
	151page
		...
		<jsp:include> : JSP안에 다른 JSP를 포함하는 경우 => 화면분할(조립식)
			...
			=> xml 형식으로 만들어져 있다
				...
				=> 속성에 값을 첨부할때는 반드시 ""
				=> 반드시 여는태그와 닫는 태그가 동일 , 독립태그 사용
			=> 동적으로 변경 (파일마다 따로 컴파일후에 HTML만 첨부)
		<jsp:useBean> : 객체 메모리할당
			=>id="" : 객체명
			=>class="" : 클래스명
		...
		<jsp:setProperty> => setter에 값을 설정
			=> name : 객체명 => useBean에서 설정된 id명과 일치가 되어야 한다
			=> property : 변수명
				  property = "name" => setName()
				  property = "sex" => setSex()
				  property = "*" => 모든 setter 메소드에 값을 채운다
		<jsp:forward page="파일명"> 
			서버에서 화면을 이동할 경우에 사용
			= sendRedirect() : URL변경 => request가 초기화
			= forward() : URL변경이 없다 => request를 가지고 있다
 --%>
<%
