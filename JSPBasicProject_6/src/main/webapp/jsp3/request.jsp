...
<%--
		...
		=================
			...
			***HttpServletRequest request 
			...
			= ASP , PHP
				 1. 서버 정보 / 브라우저 정보
				 	 http://localhost(:80 생략가능) 		/JSPBasicProject_6		/jsp3/request.jsp => URL
				 	 ==========							 ======================== URI
				 	 											     ===========      =========
				 	 													ContextPath				filename
				 	 													=> 구분자가 없다 : 파일명 자체가 구분자다
				 	 서버IP / Port => 80은 생략이 가능
...
<%--
	사용자가 전송하는 모든 데이터는 서버를 거쳐서 톰켓에 의해 request에 전송된 데이터를 묶어서 넘겨준다
																		 ======================
																		 		=> 처리를 웹 개발자가 한다
	묶는 방법 => Map 방식이다
					 ===== 키 , 값 => getParameter("키")
					 list.jsp?num=1&num=2&num=3&num=4&num=5
	setAttribute(키 , 값); 키는 중복이 불가능
		...
		<form method="post" action="request_ok.jsp">
			<table class="table">
				<tr>
					<th width=20%>이름</th>
					<td width=80%>
						<input type=text size=20 class="input-sm" name=name></td>
				</tr>
				<tr>
					<th width=20%>성별</th>
					<td width=80%>
				...
				<tr>
					...
					<td width=80%>
						...
						<input type=text  class="input-sm" name=phone2 size=20>
					</td>
				</tr>
				<tr>
					<th width=20%>소개</th>
				...
				<tr>
					...
					<td width=80%>
						...
						<input type=checkbox  name=hobby value="낚시">낚씨
