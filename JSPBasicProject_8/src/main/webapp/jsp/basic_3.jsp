...
<%--
		...
		=> 지시자
			...
			===
			  JSP에 대한 정보
			  	...
			  	에러시 이동하는 페이지 => errorPage="에러내용 출력파일"
			  	HTML출력해 두는 메모리 공간 크기 => buffer ="8kb" => 16kb
		=> 액션태그
			  <jsp:include> , <jsp:useBean> , <jsp:setProperty>
			  =========	  ========	  ==========
			  													멤버변수의 값을 채운다
			  						   클래스 메모리 할당
			  파일여러개를 무꺼서 한개의 파일로 제작
		=> 자바 / HTML을 구분
			<% %> : 일반 자바코딩 , 지역변수만 사용이 가능 , 연산자 , 제어문 , 메소드 호출 영역
			<%= %> : 변수출력 => 브라우저에 출력
		...
		=> 내장 객체
			 ...
			 response : 서버에서 요청에 대한 응답
			 				=> HTML
			 				=> Cookie
			 session : 서버에 사용자의 일부정보를 저장하는 공간
